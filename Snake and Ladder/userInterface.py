...
class Display(object):
    def __init__(self, master, img):
        ...
        self.canvas = Canvas(master, width=canvas_width,
        ...
        w = OptionMenu(self.canvas, variable, *
                       OPTIONS, command=self.get_choice)
        w.pack()
        w.place(x=740, y=225)
        ...
        self.startGame = Button(
            self.canvas, text="Start", background='white', command=self.startGame, font=("Helvetica"))
    ...
    def peices(self, move, turn):
        ...
        self.x, self.y, self.block[turn] = MatchPosition().find_snake_or_ladder(
    ...
    def create_peice(self):
        for i in range(int(self.num_player)):
            if(i == 3):
                self.x += 35
                self.y -= 105
            self.player.append(self.canvas.create_circle(
